...
import { Source, visit, print, BREAK } from 'graphql';
...
const GraphQL = (token, { cache } = {}) => {
    const customFetch = (uri, options) => {
        options.headers = Object.assign(Object.assign({}, options.headers), { 'User-Agent': 'Mozilla/5.0', 'Content-Type': 'application/json', 'X-Requested-With': 'XMLHttpRequest', Referrer: 'https://replit.com/', Cookie: token ? `connect.sid=${encodeURIComponent(token)};` : '' });
        return fetch$1(uri, options);
    };
